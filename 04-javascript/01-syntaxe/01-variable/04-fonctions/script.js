...
Tant que la focntion n'est pas appelé, le ode ne sera pas exécuté.
Une focntion peut être appelé avant ou après sa déclaration
...
salut();
/* 
Il existent d'autres façon de déclarer des fonctions,
Mais les façons suivantes, ne peuvent être appelé qu'après leurs délcarations.

On pourra déclarer une fonction dite "anonyme" et la ranger dans une variable, un objet, ou un tableau (bien que celle-ci, ne se voit jamais).
*/
...
bonsoir();
// Si trop de paramères sont fourni, il ignorera ceux en trop.
bonsoir("Maurice","Pierre")

// On peut ajouter autant de paramètres que voulu, en les séparents d'une virgule
...
// La première valeur, va au premier paramèrte, la seconde au second et ainsi de suite
...
// ? mettre fin à une fonction et retourner une valeur.

function insulte(nom) {
    if(nom === undefined)
        {
        console.error("Veuillez entrer un nom");
        // le mot clef "return" peut être utilisé pour mettre fin à la fonction
        return;
    }
    // Si le mot vlef "return" est suivi d'une valeur, celle-ci sera renvoyé au code lors de l'appel de la fonction.
 ...
 * @param {number} x Nombre à partir duquel comence le décompte
 * @returns 
 */
...
/* 
    ...
    Plein de fonctions en JS utilisent callback.
...
// Il existe d'autres fonctions de tableau fonctionnant un peu comme forEach, reduce, map...
/** 
...
//! Lorsqu'on donne une fonction en callback, il ne faut surtout pas mettre les paranthèses (à bonsoir). Ce n'est pas un appel que l'on fait, on se contente de donner le nom de notre fonction.