...
/* Math.random() génère un chiffre entre 0 et 1 Math.floor() arrondi à l'inferieur */
...
// Si elle est ViewTransition, alors ce qui se trouve entre accolade sera exécuté sinon il ne se passera rien .
if(x < 50)
{
    console.log("x est plus petit que 50");
}

// Si on a plusieurs conditions à vérifier, on peut faire suivre un if, de un ou plusieurs "else if" qui ne seront vérifié, que si toute les conditions précédentes sont fausses.
...
Il sera exécuté, si toute les conditions précédentes sont fausses.
*/
else
{
...
/* 
Si la condition, n'a qu'une seule instruciton à réliser.
On peut ne pas mettre les accolades.
*/
// Ne peut fonctionner que si on a qu'une seule chose à vérifier
...
*/
const message1 = x<=50?"x est plus êtit ou égale à 50":"x est plus grand que 50";
console.log(message1);

/* 
    On peut imbriquer des ternaires, mais si on perd en lisibilité, 
...
obj3=null;
/* 
    L'opérateur de chaînage optionnelle permet de vérifier l'existence de l'objet qui précède l'opérateur, avant de tenter de demander une propriété sur celle-ci.
*/
console.log(
    obj.info,
    obj.superinfo.a,
...
// ? Switch
// prompt permet d'afficher une modale invitant l'utilisateur à rentrer une information, cette dernière sera retourné peut être utilisée par exemple dans une variable.
const ville = prompt("De quelle ville venez-vous ?")
console.log(ville);
// if(ville === "" || ville === null) ville = "sans réponses";
