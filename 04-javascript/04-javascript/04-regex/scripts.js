"use strict";
/* 
Les Regex ou Regular Expression permettent de rechercher la présence de caractères dans un string.
...
/* 
    ...
    La première que l'on va voir est "maRegex.test(monString);"
    qui retourne un boolean selon si la regex a trouvé ce qui était recherché

La première regex va vérifier la présence de 'ou'
dans le string
*/
console.log(r1, r1.test("Bonjour"), r1.test("Salut"));
// r2 va vérifier la présence d'un des caractères "o" ou "u"
...
const r4 = /ou$/;
// $ indique que ce la doit terminer par "ou"
console.log(r4, r4.test("Bonjour"), r4.test("mou"));

const r5 = /ou|oi/;
// | est un "or", ici on veut dire soit "ou" soit "oi"
console.log(r5, r5.test("Bonjour"), r5.test("Bonsoir"));

const r6 = /[a-z]/; 
// const r6 = /[a-zA-Z]/; 
// pour prendre les majuscules
// vérifie la présence de caractère entre a et z (donc n'importe quelle lettre de l'alphabet)
console.log(r6, r6.test("Bonjour"), r6.test("0645785426"));


const r7 = /[^a-z]/; 
// "^" entre "[]" indique que les caractères ne doivent pas être présent 
console.log(r7, r7.test("bonjour"), r7.test("0645785426"));

const r8 = /(ou)?/;
// le "?" indique que l'élément précédent doit être présent 1 ou 0 fois
console.log(r8, r8.test("Bonjour"), r8.test("Pizza"));

const r9 = /(ou)+/;
// le "+" indique que l'élément précédent doit être présent 1 ou + fois.
console.log(r9, r9.test("Bonjour"), r9.test("Pizza"));

const r10 = /(ou)*/;
// le "*" indique que l'élément précédent doit être présent 0 fois ou +.
...
const r13 = /(cou){2,}/;
// Sans chiffre après la virgule, on indique un minimum de 2 fois, mais pas de maximum.
console.log(r13, r13.test("coucou"), r13.test("coup"));

const r14 = /\^/;
// On peut utiliser "\" pour échapper un caractère et lui faire perdre son effet habituel.
// Ici je cherche juste "^"
console.log(r14, r14.test("coucou^_^"), r14.test("coucou-_-"));

const r15 = /\s/;
// const r15 = / /;
// Vérifie la présence d'un espace ("\" peut aussi donner un effet à un caractère qui n'en a normalment pas)
...
// "\" suivi d'un chiffre, indique que l'on souhaite voir le même contenu qu'une paranthèse précédente. 
//Ici, si il trouve un "ou" il veut un autre "ou", si il trouve un "oi", il veut autre "oi"
console.log(r17, r17.test("Bonjour à tous"), r17.test("bonsoir à tous"));

//  ? Les flags

/* 
    Les flags sont des caractères qui viennent se placer à la fin d'une regex pour lui donnner une nouvelle capacités.

    On va les tester sur les méthodes "string.match(regex)" qui retourne un tableau contenant les résultats de la regex.
*/

const phrase = "J'aime la pizza, les cannelés et les okonomiyaki";
console.log(phrase.match(/pizza/));
// par défaut, les regex s'arrêtent en premier résultat trouvé.
...
// Par défaut, les regex sont sensible à la casse (minuscule et majuscule)
...
éème : Paul
3ème: Charlie`;
// Mon string commence par un chiffre, mais les atus à la ligne ne sont pas des strings différent. Donc je n'ai qu'un résultat.
